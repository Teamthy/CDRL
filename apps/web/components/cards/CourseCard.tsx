...
export default function CourseCard({ course, href }: Props) {
    ...
    return (
        <Link href={target} className="course-card course-card-img" aria-label={`${course.title} ${course.subtitle}`}>
            <div
                className="course-card-bg"
                aria-hidden="true"
                style={{ backgroundImage: `url(${courseImageFor(course.track, course.slug)})` }}
            />
